...
from face_processor import load_model, get_embeddingimport streamlit as st 
import numpy as np
from PIL import Image
import cv2
from firebase_utils import db, bucket
...
if not students:
    st.warning("⚠ No student list for this class. Ask admin to upload CSV.")
    st.stop()
...
if student.get("face_registered"):
    st.error("❌ Face already registered. Contact admin to reset.")
    st.stop()
...
# ---- FIXED SECTION ---- prevents crash when step==5
if st.session_state.step < len(POSES):
    ...
    st.markdown("📌 Only one face must be visible. Good lighting recommended.")
    ...
    if img_file and st.button("Save This Pose"):
        ...
        if is_blurry(img_np): st.error("⚠ Blurry image. Try again."); st.stop()
        ...
        st.success(f"✔ {current_pose} Captured Successfully")
        st.rerun()
...
# ------------------------ FINAL SUBMISSION ------------------------
    ...
    if st.button("Finalize Registration 🚀"):
        ...
        # save embedding inside class student record
